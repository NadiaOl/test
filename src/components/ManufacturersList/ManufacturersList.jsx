...
export const ManufacturersList = () => {
    ...
    try {
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          currancy: editingItem.currancy || 'USD',
        }),
      });
      ...
      if (isEdit) {
        setManufacturers(prev =>
          prev.map(m =>
            m._id === editingItem.id ? { ...m, ...editingItem } : m
          )
        );
      } else {
  ...
  const deleteManufactor = async m => {
    if (window.confirm(`Видалити ${m.name}?`)) {
      await fetch(
        `https://suppliers-backend-nphe.onrender.com/api/manufacturers/${m._id}`,
        {
          method: 'DELETE',
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const handleProductSave = async () => {
    ...
    try {
      ...
      setManufacturers(prev =>
        prev.map(m => (m._id === currentManId ? updatedMan : m))
      );
  ...
  const deleteProduct = async (manId, product) => {
    if (window.confirm(`Видалити ${product.name}?`)) {
      const res = await fetch(
        `https://suppliers-backend-nphe.onrender.com/api/manufacturers/${manId}/products/${product._id}`,
        {
          method: 'DELETE',
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      setManufacturers(prev =>
        prev.map(m =>
          m._id === manId
            ? { ...m, products: updatedData.products || updatedData }
            : m
        )
      );
    }
      ...
      setManufacturers(prev =>
        prev.map(m => {
          if (m._id === currentManId) {
            if (isEdit) {
              const updatedContacts = (m.contacts || []).map(c =>
                c._id === editingContact.id ? savedContact : c
              );
  ...
  const deleteContact = async (manId, contact) => {
    if (window.confirm(`Видалити ${contact.fullName}?`)) {
      try {
        await fetch(
          `https://suppliers-backend-nphe.onrender.com/api/manufacturers/${manId}/contacts/${contact._id}`,
          {
            method: 'DELETE',
            headers: { Authorization: `Bearer ${token}` },
          }
        ...
        setManufacturers(prev =>
          prev.map(m => {
            if (m._id === manId) {
              return {
                ...m,
                contacts: (m.contacts || []).filter(c => c._id !== contact._id),
              };
            }
            return m;
          })
    ...
    <Container>
          ...
          <ProductContainer>
            <CardDiv>
              <Wrapper>
                ...
                <BBlock>
                  <ButtonsGroup onClick={() => openContactList(m)}>
                    <Contacts />
                  </ButtonsGroup>
                  <ButtonsGroup onClick={() => openEditModal(m)}>
                    <Edit />
                  </ButtonsGroup>
                  <ButtonsGroup onClick={() => deleteManufactor(m)}>
                    <Delete />
                  </ButtonsGroup>
                </BBlock>
              </Wrapper>
                ...
                <tbody>
                  {m.products?.map(p => (
                    <tr key={p._id}>
                      ...
                      <Td>
                        {p.fact && p.plan
                          ? Math.ceil((p.fact * 100) / p.plan)
                          : '-'}
                      </Td>
                      <Td>
                        <ButtonsGroup
                          style={{ marginRight: '4px' }}
                          onClick={() => openProductModal(m._id, p)}
                        >
                          <Edit />
                        </ButtonsGroup>
                        <ButtonsGroup
                          style={{ marginLeft: '4px' }}
                          onClick={() => deleteProduct(m._id, p)}
                        >
                          <Delete />
                        </ButtonsGroup>
                      </Td>
            ...
            <Wrapper2>
              <Button onClick={() => openProductModal(m._id)}>
                Додати продукт
              </Button>
            </Wrapper2>
          </ProductContainer>
      ...
      {isContactListModalOpen && currentManufacturer && (
        ...
        >
              ...
              {currentManufacturer.contacts?.map(c => (
                <tr key={c._id}>
                  ...
                  <Td>
                    ...
                    <ButtonsGroup
                      style={{ marginLeft: '4px' }}
                      onClick={() => deleteContact(currentManId, c)}
                    >
                      <Delete />
                    </ButtonsGroup>
                  </Td>
          ...
          <Wrapper2>
            <Button onClick={() => openContactEditModal()}>
              Додати контакт
            </Button>
          </Wrapper2>
        </Modal>
      ...
      {isModalOpen && (
        <Modal
          title={editingItem.id ? 'Редагувати' : 'Додати'}
          onSave={handleSave}
        ...
        >
          ...
          <Field>
            ...
            <Input
              ...
              onChange={e =>
                setEditingItem({ ...editingItem, buyer: e.target.value })
              }
            />
          </Field>
          <Field>
            ...
            <Input
              ...
              onChange={e =>
                setEditingItem({ ...editingItem, currancy: e.target.value })
              }
            />
          </Field>
        </Modal>
      ...
      {isProductModalOpen && (
        ...
        >
          ...
          <Field>
            ...
            <Input
              type="number"
              value={editingProduct.gap}
              onChange={e =>
                setEditingProduct({ ...editingProduct, gap: e.target.value })
              }
            />
          </Field>
          <Field>
          ...
          <Field>
            ...
            <Input
              ...
              onChange={e =>
                setEditingProduct({ ...editingProduct, foc: e.target.value })
              }
            />
          </Field>
          <Field>
            ...
            <Input
              ...
              onChange={e =>
                setEditingProduct({ ...editingProduct, plan: e.target.value })
              }
            />
          </Field>
          <Field>
            ...
            <Input
              ...
              onChange={e =>
                setEditingProduct({ ...editingProduct, fact: e.target.value })
              }
            />
          </Field>
        </Modal>
